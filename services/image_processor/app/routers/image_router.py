...
@router.post("/")
async def process_image(width: Annotated[int, Form()], height: Annotated[int, Form()], file: UploadFile = File(...),  service: ImageService = Depends(ImageService)) -> ImageResponse:
    """
    Sube una imagen para procesarla
    """
    return await service.process_image(file, width, height)
